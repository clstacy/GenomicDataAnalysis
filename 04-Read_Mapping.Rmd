...
```{r ready-packages-mapping, cache=FALSE}
...
```{r align-reads-mapping, eval=FALSE}
...
```{bash create-condaEnvironment-mapping, engine.opts='-l', eval=FALSE}
...
```{bash fetch-transcriptome-mapping}
# Define the destination file path
...
```{bash index-salmon-mapping, engine.opts='-l', message=FALSE}
...
```{r multimap-align-mapping, eval=FALSE}
