...
WT.graph <- graph.data %>%
  ggplot(aes(x = x, y = y)) +
  overlay_raw_map(pathway_to_graph, transparent_colors = c("#cccccc","#FFFFFF","#BFBFFF","#BFFFBF")) +
...
msn24.graph <- graph.data %>%
  ggplot(aes(x = x, y = y)) +
  overlay_raw_map(pathway_to_graph, transparent_colors = c("#ffffff"))+
  ggrepel::geom_label_repel(
...
TESTING:::
```{r}
g <- pathway(pathway_to_graph,group_rect_nudge=0) ## Obtain pathway graph (don't show grouping rect)
gg <- ggraph(g, layout="manual", x=x, y=y)+
  ...
  geom_node_rect(color="white",fill="white",aes(filter=type=="gene"))+
  geom_node_rect(color="white",fill="white",aes(filter=type=="ortholog"))+
  theme_void()
gg
```
